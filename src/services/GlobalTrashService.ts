import { FileType, TrashItem } from '../SmartTrashSystem';
...
class GlobalTrashService {
  ...
  private restoreHandlers: Map<string, (item: TrashItem) => Promise<boolean>> = new Map();
  ...
  // Register a restore handler for a given category (e.g. 'email', 'memory', 'contacts')
  registerRestoreHandler(category: string, handler: (item: TrashItem) => Promise<boolean>) {
    this.restoreHandlers.set(category, handler);
  }

  // Add item to trash
  addItem(item: Omit<TrashItem, 'id' | 'deletedAt'> & { category?: string; payload?: any }) {
    const trashItem: TrashItem = {
      ...item,
      id: Math.random().toString(36).substring(2, 9),
      deletedAt: Date.now(),
      category: (item as any).category || 'generic',
  ...
  // Remove item from trash (attempt restore via registered handlers)
  async removeItem(id: string) {
    const item = this.items.find(i => i.id === id);
    if (!item) return;

    const category = (item as any).category || 'generic';
    const handler = this.restoreHandlers.get(category);
    if (handler) {
      try {
        ...
        console.warn('[GlobalTrashService] Restore handler failed for', category, e);
      }
    }

    // Fallback: remove without restoring
    this.items = this.items.filter(i => i.id !== id);
    this.notifyListeners();
    this.saveToStorage();
  }
