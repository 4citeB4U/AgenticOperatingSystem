...
const SmartTrashSystem: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (window as any).closeTrashSystem = () => setIsOpen(false);
  }, []);
      ...
      <div className="flex-none bg-zinc-950 border-b border-white/5 shadow-2xl z-50">
        ...
        <div className="p-4 max-w-xl mx-auto flex flex-col gap-4">
          ...
          <form onSubmit={handleSendMessage} className="relative group">
            ...
            />
            <button type="submit" className="absolute right-3 top-3 bottom-3 w-14 bg-cyan-500 rounded-2xl text-black flex items-center justify-center shadow-lg hover:bg-cyan-400 active:scale-90 transition-all">
          ...
          {items.map(item => (
            <div key={item.id} className="p-6 bg-zinc-900/40 border border-white/5 rounded-[3rem] hover:border-cyan-500/20 transition-all shadow-lg group">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <button onClick={() => { globalTrashService.removeItem(item.id); speakMessage(`${item.name} has been restored!`); }} className="py-3 bg-cyan-500/10 rounded-2xl text-[9px] font-black uppercase text-cyan-400 hover:bg-cyan-500 hover:text-black transition-all">Restore</button>
                <button onClick={() => { globalTrashService.permanentlyDeleteItem(item.id); speakMessage("Shredding complete."); }} className="py-3 bg-red-600/10 rounded-2xl text-[9px] font-black uppercase text-red-500 hover:bg-red-600 hover:text-white transition-all">Destroy</button>
              </div>
            </div>
          ))}
      ...
      {viewing && (
        <div className="fixed inset-0 z-[300] bg-black/95 flex flex-col animate-in fade-in slide-in-from-bottom-12 duration-500">
          <div className="p-8 flex justify-between items-center border-b border-white/5 bg-zinc-950/80 backdrop-blur-xl">
            ...
            <button onClick={() => setViewing(null)} className="w-14 h-14 bg-white/5 rounded-full flex items-center justify-center text-white/20 hover:text-white transition-all border border-white/5">
          ...
          <div className="p-10 border-t border-white/5 bg-zinc-950 grid grid-cols-2 gap-8 pb-20 shadow-[0_-30px_60px_rgba(0,0,0,0.8)]">
            <button 
              onClick={() => { globalTrashService.removeItem(viewing.id); setViewing(null); speakMessage("Restoring data segment!"); }} 
